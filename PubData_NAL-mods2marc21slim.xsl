...
<xsl:stylesheet version="1.0"
	...
	<!-- updated on 06/27/2012 by Chuck Schoppet changed order of 773 subfields -->
           <!-- Updated on 5/24/2019 by Emily Somach -->

<!--  <xsl:include href="file:///MARC21slimUtils.xsl"/>-->
...
<!-- changed by CD -->
      ...
      <xsl:call-template name="ctrl006"/>
      <xsl:call-template name="ctrl007"/>
      ...
      <marc:controlfield tag="008">
        ...
        <xsl:choose>
          ...
          <xsl:when test="typeOfResource='text' and genre='article'">o</xsl:when>
          <xsl:otherwise>|</xsl:otherwise>
        </xsl:choose>
        <!-- 24-27 -->
        <xsl:choose>
      ...
      <xsl:apply-templates/>
      <xsl:call-template name="datafield300s"/>
  ...
  <xsl:template match="extent">
    ...
    <xsl:template name="datafield300s">
  ...
  <xsl:template match="accessCondition">
   ...
   <!-- added 5/19 -->
    <xsl:template name="ctrl006">
        <marc:controlfield tag="006">m     o  d        </marc:controlfield>
    </xsl:template>
    
    <xsl:template name="ctrl007">
...
<!-- Changed by CD -->
  <xsl:template match="location[url]">
    <xsl:for-each select="url">
      <xsl:choose>
        <xsl:when test="starts-with(text(), 'http://handle.nal.usda.gov')">
      <xsl:call-template name="datafield">
        <xsl:with-param name="tag">856</xsl:with-param>
...
<!-- changed by CD -->
  ...
  <!-- LC note 930, bad string processiong functions in xslt 1.0, will code values be uniform?  and follow the same string pattern?  -->
  ...
  <!-- LC note 945, bad string processiong functions in xslt 1.0, will code values be uniform?  and follow the same string pattern? -->
