...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xd="http://www.oxygenxml.com/ns/doc/xsl" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:f="http://functions" xmlns:saxon="http://saxon.sf.net/" xmlns:ali="http://www.niso.org/schemas/ali/1.0/" xmlns="http://www.loc.gov/mods/v3" exclude-result-prefixes="xd xs f saxon xlink xsi xml ali">
    ...
    <xsl:output method="xml" encoding="UTF-8" name="archive-original" version="1.0" doctype-public="-//NLM//DTD JATS (Z39.96) Journal Publishing DTD v1.1 20151215//EN" doctype-system="https://jats.nlm.nih.gov/publishing/1.1/JATS-journalpublishing1.dtd"/>
    ...
    <xd:doc scope="stylesheet" id="brill">
        <xd:desc>
            ...
            <xd:p><xd:b>Edited on:</xd:b>Aug 20, 2021</xd:p>
            ...
            <xd:ul>
                ...
                <xd:li>
                    <xd:p><xd:b>Issue #2</xd:b>: Brill's metadata rendered two mods
                            <xd:i>dateIssued</xd:i> tags containing a  <xd:i>keyDate</xd:i>
                        attribute and its corresponding value set to "yes" (e.g.,
                            <xd:i>keyDate</xd:i>="yes")</xd:p>
                    <xd:p><xd:b>Solution</xd:b>: This stylesheet uses the named template <xd:a docid="brill_originInfo">brill_originInfo</xd:a> to choose only one date
                        based on the conditions contained therein.</xd:p>
                </xd:li>
                <xd:li>
                    <xd:p><xd:b>Issue #3:</xd:b>Text type tags appearing within the <xd:i>mods
                            relatedItem/part</xd:i> subelements correspond with the
                            <xd:i>dateIssued</xd:i> tag. Thus, similar logic to select the
                            <xd:i>dateIssued</xd:i> tag is applied to a named template, <xd:a docid="brill_modsPart">brill_modsPart</xd:a>. </xd:p>
                    <xd:p><xd:b>Solution: </xd:b> Applying similar conditional logic this stylesheet
                        chooses the corresponding date to the <xd:i>dateIssued</xd:i> tag.
                        </xd:p><xd:p>For example:</xd:p>
            ...
            <xd:ul><xd:p><xd:b>Enhancements:</xd:b> Recommended changes for more efficient processing</xd:p>
                <xd:li><xd:p><xd:b>Enhancement #1</xd:b>: Pairing the affiliation to the correct
                        author is overly complicated in <xd:a href="../jats_to_mods_30.xsl">jats_to_mods_30.xsl</xd:a>
                </xd:p><xd:b>Solution:</xd:b> 
                    <xd:a docid="brill-authors-name-info">brill-authors-name-info</xd:a> has been
                    edited to used an XPath expression to match aff[@id] (i.e. the affiliation) to
                    xref/@rid (i.e., author's name id) using the <xd:i>current() function</xd:i>.
                            <xd:p><xd:b>XPath expression</xd:b>: (e.g.,
                        aff[@id=current()/xref/@rid]) </xd:p></xd:li>
            </xd:ul>
        </xd:desc>
    </xd:doc>
    <xsl:template match="/">
        ...
        <xsl:result-document method="xml" encoding="UTF-8" indent="yes" href="file:///{$workingDir}N-{replace($originalFilename, '(.*/)(.*)(\.xml)', '$2')}_{position()}.xml">
            <mods version="3.7">
                ...
                <xsl:apply-templates select="article/front/article-meta/article-id[@pub-id-type] | /article/front[1]/article-meta[1]/doi-group[1]/article-doi[1]"/>
            ...
            <xsl:for-each select="/article/front/article-meta">
                ...
                <xsl:choose>
                    <xsl:when test="pub-date[@publication-format = 'online' and @date-type = 'article']">
                        <xsl:apply-templates select="pub-date[@publication-format = 'online' and @date-type = 'article']" mode="dateIssued"/>
                    </xsl:when>
                    <!--condition 2-->
                    <xsl:when test="pub-date[@publication-format = 'online' and not(@date-type = ('issue', 'article'))]">
                        <xsl:apply-templates select="pub-date[@publication-format = 'online' and not(@date-type = ('issue', 'article'))]" mode="dateIssued"/>
                    </xsl:when>
                    <!--condition 3-->
                    <xsl:when test="pub-date[@date-type = 'issue' and @publication-format = 'online']">
                        <xsl:apply-templates select="pub-date[@date-type = 'issue' and @publication-format = 'online']" mode="dateOther"/>
                    </xsl:when>
                    <!--condition 4-->
                    <xsl:when test="not(pub-date)">
                        <xsl:apply-templates select="history/date[@date-type = 'accepted']" mode="dateIssued"/>
                        <!--condition 5-->
                        <xsl:if test="not(//history/date[@date-type = 'accepted'])">
                            <xsl:apply-templates select="permissions/copyright-year" mode="dateIssued"/>
                        </xsl:if>
                    </xsl:when>
                    <xsl:otherwise>
    ...
    <xd:doc scope="component" id="brill_modsPart">
        <xd:desc>
            <xd:p>Similar to the purpose of the modsPart template in the <xd:a href="../jats_to_mods_30.xsl"> main JATS to MODS stylesheet</xd:a> this template
    ...
    <xsl:template name="brill_modsPart">
        <part>
            ...
            <xsl:choose>
                <!--conditoins 1-3-->
                <xsl:when test="/article/front/article-meta/pub-date[@publication-format = 'online' or @date-type = ('issue', 'article')]">
    ...
    <xsl:template match="//pub-date[@publication-format = 'online' or @date-type = ('issue', 'article')][1]" mode="brill_modsPart">
